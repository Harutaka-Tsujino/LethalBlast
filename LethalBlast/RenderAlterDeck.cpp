...
void RenderAlterDeck(ImagesCustomVertex* pChoiseWordCollisionsVertex, ImagesCustomVertex* pDeckComponentCollisionsVertex, ImagesCustomVertex* pSkillInfo,
	CustomVertex* endDeckAlterVertices,CustomVertex* pBackgroundVertices, CustomVertex* pWordDatasBackVertices,PLAYERTYPE* pPlayerType,
	TEXTUREID* wordTexIds, WordData* pPlayerWordDatas, Deck* pPlayerDecks, int* pDeckNumToAlter, bool* clickedWord, bool* pChoiceWordData)
{
	...
	case WEAPON_MASTER:
	{
		...
		if (frameCount == INIT_FRAME)
		{
			...
			RoadTexture("Texture/AlterDeck/DeckAlterWeaponMasterBackground1.png", &alterDeckTexId[WM_ALTER_BACK_1_TEX]);
			RoadTexture("Texture/AlterDeck/DeckAlterWeaponMasterBackground2.png", &alterDeckTexId[WM_ALTER_BACK_2_TEX]);
			RoadTexture("Texture/AlterDeck/DeckAlterWeaponMasterBackground3.png", &alterDeckTexId[WM_ALTER_BACK_3_TEX]);
		...
		for (int wordDatas = 0; wordDatas < WEAPON_MASTER_WORD_MAX; ++wordDatas)
		{
			if (pChoiceWordData[wordDatas] == true)
			{
				...
				for (int costConfig = 1;costConfig < 3;++costConfig)
				{
					...
					WriteText(DISPLAY_WIDTH * 0.83, DISPLAY_HEIGHT * 0.2, cost, DT_RIGHT, fontId[0],0xFF000000);
					WriteText(DISPLAY_WIDTH * 0.87, DISPLAY_HEIGHT * 0.2, "/12", DT_RIGHT, fontId[1], 0xFF000000);
				}
		...
		WriteText(DISPLAY_WIDTH*0.26, DISPLAY_HEIGHT*0.7, deckCostMax, DT_RIGHT, fontId[1]);
